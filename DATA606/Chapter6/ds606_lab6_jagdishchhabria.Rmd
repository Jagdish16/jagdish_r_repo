---
title: ds606_Inferenceforcategoricaldata_lab6_jagdishchhabria'
output:
  html_document:
    css: ./lab.css
    highlight: pygments
...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    These appear to be sample statistics.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    We must assume that the sample was randomly drawn. At 50,000 respondents, the size is large enough but still less than 10% of the global population.
...
Each row corresponds to the survey respondents for a given country. Each row of atheism gives the proportion of respondents from the total surveyed in that country, that identified themselves as convinced atheists.
...
The conditions of inference are that the sample should be randomly drawn, and that there should be at least 10 yes (i.e. atheists) and 10 no (i.e. non-atheists) answers.
...
6.  Based on the R output, what is the margin of error for the estimate of the 
    ...
    Margin of error = 0.0634 - 0.0499 = 0.0135
    Margin of error = 1.96 * sqrt((0.0499 X 0.9501)/1002)
...
The conditions for inference (random sample, less than 10% of population, at least 10 successes and 10 failures) are met in both the cases mentioned above - India and Japan.



```{r}
(me.india<-0.0404-0.0302)

(me.japan<-0.3329-0.3069)
```

Margin of Error for India: 0.0102
Margin of Error for Japan: 0.026
...
The relation between p and me is non-linear. It is quadratic because the numerator for SE has a p-squared term in it. The ME reaches its higest point when p=0.5, because that gives the largest value of the numerator in the SE term. All other values of p before and after 0.5 yield lower values of the ME.
...
The graph shows that the sampling distribution is normal.
...
for(i in 1:5000){
  ...
  p2_hats[i] <- sum(samp2 == "atheist")/n2
}
...
for(i in 1:5000){
  ...
  p3_hats[i] <- sum(samp3 == "atheist")/n3
}
...
for(i in 1:5000){
  ...
  p4_hats[i] <- sum(samp4 == "atheist")/n4
}

par(mfrow = c(2, 2))

hist(p_hats, main = "p = 0.1, n = 1040", xlim = c(0, 0.18))
hist(p2_hats, main = "p2 = 0.1, n = 400", xlim = c(0, 0.18))
hist(p3_hats, main = "p3 = 0.02, n = 1040", xlim = c(0, 0.18))
hist(p4_hats, main = "p4 = 0.02, n = 400", xlim = c(0, 0.18))
...
While p impacts the center of the sampling distribution as can be expected, n controls the spread i.e. as n grows, the spread decreases.
...
inference(au12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
#inference(ec12$response, est = "proportion", type = "ci", method = "theoretical",           success = "atheist")

```
Based on the above, it can be seen that in the case of Ecuador, there are less than 10 successes, which violates the conditions for inference and therefore this approach is not appropriate for reporting the margin of error for Ecuador.
...
```
The confidence intervals for proportion of aethists in Spain in 2005 and 2012 overlap,
...
p.12=0.0499
n12=1002
(p.pooled=(10+50)/(1002+1002))
under=((p.pooled*(1-p.pooled))/n05)+((p.pooled*(1-p.pooled))/n12)
(z.us=(p.05-p.12)/sqrt(under))


```
Given this z-statistic, the p-value is almost 0, and the null hypothesis (no change in proportion of atheists in the USA between 2005 and 2012) can be rejected. 
...
39*0.05
```
we'd expect to detect a change simply by chance in 1 country.
...
```
I would have to sample at least 9,604 people.
