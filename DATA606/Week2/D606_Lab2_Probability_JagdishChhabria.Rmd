---
title: "Data606_Lab2_JagdishChhabria"
...
A streak length of 1 means 1 hit followed by 1 miss. A streak length of 0 has 1 miss only.
...
The distribution of Kobe's streak lengths from the 2009 NBA finals is right-skewed. It's median is 0 and mean is 0.76, and the maximum is 4 hits i.e. his longest streak had 4 baskets in a row, which ocurred once.
...
Only 17 flips came up heads in the simulation of 100 tosses of the unfair coin.
...
independent_streak <- calc_streak(sim_basket1)
...
The distriution of independent streaks is also right-skewed, but it's mean at 0.675 hits is lower than that of Kobe's streaks. While it also had a maximum of 4 hits in a row, the frequency of that type of streak was higher than in Kobe's streaks. On the other hand, the independent shooter had fewer single-hit streaks and 3-hot streaks than Kobe.

How long is the player's longest streak of baskets in 133 shots?
    
The longest is 4 hits.

If you were to run the simulation of the independent shooter a second time, how would you expect its streak distribution to compare to the distribution from the question above? Exactly the same? Somewhat similar? Totally different? Explain your reasoning.

I would expect the second simulation of the independent shooter to be somewhat similar to the first simulation, since the probability of a hit is the same. I would be very surprised if the distribution was exactly the same or totally different.

```{r}
outcomes <- c("H", "M")
sim_basket2 <- sample(outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55))
table(sim_basket2)
independent_streak <- calc_streak(sim_basket2)
barplot(table(independent_streak))
independent_streak
summary(independent_streak)
```
When I did simulate the indepdendent shooter a second time, I did see more variability than I expected. The mean for this simulation was 0.952, and the maximum was a streak of 7 hits which I found quite surprising.


How does Kobe Bryant's distribution of streak lengths compare to the distribution of streak lengths for the simulated shooter? Using this comparison, do you have evidence that the hot hand model fits Kobe's shooting patterns? Explain.

Kobe's distribution had a lower mean and a lower maximum than the independent shooter distribution. This refutes the categorization of Kobe's streak as belonging to a "hot hand model". On a lighter note, it may actually be a candidate for a "cold hand model".
